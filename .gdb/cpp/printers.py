...
class StdListPrinter:
    ...
    def to_string(self):
        if (
            self.val["_M_impl"]["_M_node"].address
...
class StdExpAnyPrinter(SingleObjContainerPrinter):
    ...
    def __init__(self, typename, val):
        ...
        if mgr != 0:
            ...
            rx = r"""({0}::_Manager_\w+<.*>)::_S_manage\({0}::_Op, {0} const\*, {0}::_Arg\*\)""".format(
                typename,
            )
...
def register_type_printers(obj):
    ...
    for ch in ("", "w", "u16", "u32"):
        ...
        add_one_type_printer(obj, "__cxx11::basic_string", "__cxx11::" + ch + "string")
    ...
    for ch in ("", "w", "u16", "u32"):
        add_one_type_printer(obj, ns + "basic_string_view", ns + ch + "string_view")
...
def build_libstdcxx_dictionary():
    ...
    libstdcxx_printer.add("std::__debug::priority_queue", StdStackOrQueuePrinter)
